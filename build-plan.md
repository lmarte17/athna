...
| Item             | Detail                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Task**         | For complex intents, decompose into ordered subtasks with checkpointing                                                                                                                                                                                                                                                                                                                        |
| **Actions**      | When a user intent implies > 2 steps, the Navigator Engine first generates a subtask plan (e.g., "1. Navigate to site, 2. Search for product, 3. Filter by price, 4. Extract top 3 results"). Each subtask is tracked with status: PENDING, IN_PROGRESS, COMPLETE, FAILED. Checkpoint after each subtask completion. On failure, retry from the last successful checkpoint (not from scratch). |
| **Acceptance**   | "Find the cheapest flight from NYC to London next Friday on Google Flights" decomposes into ~5 subtasks. Failure at step 4 retries from step 4, not step 1. Subtask list is visible in task status feed.                                                                                                                                                                                       |
...
| **Actions**      | Network errors, JS runtime errors, CDP errors, and timeout errors are all caught by the orchestration layer. Convert to: `{type: NETWORK                                                                            | RUNTIME | CDP | TIMEOUT, status: number | null, url: string, message: string, retryable: boolean}`. Navigator Engine receives these instead of error page screenshots. Engine decides: retry, navigate away, or escalate to screenshot-based approach. |
